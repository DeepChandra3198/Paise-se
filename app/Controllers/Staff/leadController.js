...
async function index(req, res) {
    ...
    return res.render("staff/lead/index", {
        leads,
        type
    });
}
...
async function getById(req, res) {
    ...
    const bankName = await prisma.bankLoanTypeCommission.findMany({
        orderBy: {
            bank: 'asc'
        }
    })

    return res.render("staff/lead/view", {
        lead,
        bankName
    });
}
...
async function updateStatus(req, res) {
    ...
    const { id } = req.params
    const { disbursedAmount, leadRemark, bankName, status, loanType } = req.body
