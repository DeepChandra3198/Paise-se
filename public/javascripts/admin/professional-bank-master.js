...
$(document).ready(function () {
  $('.bank-select-box').select2({
    ...
    dropdownParent: $('#createProfessionalBankMasterModal'),
  });
  $('.edit-bank-select-box').select2({
    placeholder: 'Search for company',
    width: '100%',
    dropdownCssClass: 'max-z-index',
    dropdownParent: $('#editProfessionalBankMasterModal'),
  });
});

document
  .querySelector('.professional-bank-master-form')
  .addEventListener('submit', async (event) => {
    ...
    try {
      const response = await axios.post('/admin/setup/professional-bank-master', formData);
...
document
  .querySelector('.edit-professional-bank-master-form')
  .addEventListener('submit', async (event) => {
    event.preventDefault();
    ...
    try {
      const response = await axios.patch(
        `/admin/setup/professional-bank-master/update/${companyBankMasterId}`,
        formData
      );
      const { status, message } = response.data;
...
document.querySelectorAll('.edit-professional-bank-master-btn').forEach((btn) => {
  ...
  .addEventListener('submit', async (event) => {
    ...
    try {
      const response = await axios.post(
        '/admin/setup/professional-bank-master-via-excel',
        formData,
        {
          headers: {
