...
$(document).ready(function () {
  $('.bank-select-box').select2({
    ...
    dropdownParent: $('#createOtherBankMasterModal'),
  });
  $('.edit-bank-select-box').select2({
    placeholder: 'Search for company',
    width: '100%',
    dropdownCssClass: 'max-z-index',
    dropdownParent: $('#editOtherBankMasterModal'),
  });
});

document.querySelector('.other-bank-master-form').addEventListener('submit', async (event) => {
  event.preventDefault();
  let formData = new FormData(event.target);
  // this is the formData object with the form data key value pairs
  formData = Object.fromEntries(formData.entries());
  try {
    const response = await axios.post('/admin/setup/other-bank-master', formData);
...
document.querySelector('.edit-other-bank-master-form').addEventListener('submit', async (event) => {
  event.preventDefault();
  ...
  try {
    const response = await axios.patch(
      `/admin/setup/other-bank-master/update/${companyBankMasterId}`,
      formData
    );
    const { status, message } = response.data;
...
document.querySelectorAll('.edit-other-bank-master-btn').forEach((btn) => {
  btn.addEventListener('click', async (editBtn) => {
    ...
    document.querySelector('.max-loan-amount').value = companyBankMaster.maxLoanAmount;
    document.querySelector('.redirect-url').value = companyBankMaster?.redirectUrl;
  });
});
...
document
  ...
  .addEventListener('submit', async (event) => {
    ...
    try {
      const response = await axios.post('/admin/setup/other-bank-master-via-excel', formData, {
        headers: {
