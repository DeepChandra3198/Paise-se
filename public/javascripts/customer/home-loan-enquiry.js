...
document.querySelectorAll('.loan-type').forEach((empType) => {
  empType.addEventListener('change', (type) => {
    const loanType = type.target.value;
    switch (loanType) {
      case 'Home Loan':
        document
          .querySelector('.tentative-purchase-month')
          .parentElement.classList.remove('d-none');
        break;
      case 'Loan Against Property':
        document.querySelector('.tentative-purchase-month').parentElement.classList.add('d-none');
        break;
    }
  });
});
...
document.querySelector('.home-loan-enquiry-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const response = await axios.post(`/home-loan-enquiry/${productId}`, formData);
    Swal.fire({
